...
{
  // テクセルサイズを計算
  vec2 oneTexel = 1 / vec2(textureSize(textureDiff, 0));

  // 明るい成分を計算し、4x4ピクセルの縮小ぼかし処理を行う
  outColor.rgb = GetBrightnessPart(texture(textureDiff, inTexcoord).rgb) * 4;
  outColor.rgb += GetBrightnessPart(
    texture(textureDiff, inTexcoord + vec2(-oneTexel.x, oneTexel.y)).rgb);
  outColor.rgb += GetBrightnessPart(
    texture(textureDiff, inTexcoord + vec2( oneTexel.x, oneTexel.y)).rgb);
  outColor.rgb += GetBrightnessPart(
    texture(textureDiff, inTexcoord + vec2(-oneTexel.x,-oneTexel.y)).rgb);
